...
SetClockXTAL
		...
		; by disabling RCC2 and setting RCC for PLL bypass / 16 MHz
        ;
        LDR     R0, = SYSCTL_RCC_R
		LDR     R1, [R0]
		AND     R1, #:NOT:(SYSCTL_RCC_XTAL_M)
		AND     R1, #:NOT:(SYSCTL_RCC_OSCSRC_M:OR:SYSCTL_RCC_IOSCDIS:OR:SYSCTL_RCC_MOSCDIS)
		ORR     R1, #SYSCTL_RCC_PWRDN:OR:SYSCTL_RCC_BYPASS:OR:SYSCTL_RCC_XTAL_16MHZ
		ORR     R1, #SYSCTL_RCC_OSCSRC_MAIN:OR:SYSCTL_RCC_IOSCDIS
		STR     R1, [R0]
